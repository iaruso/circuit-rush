# Circuit Rush

> ⚠️ **Important Notice**  
> This version will be **removed from public access** once the initial release of the renewed version is ready.  
> If you wish to use or review this legacy version, please clone the repository before removal.

### Welcome to Circuit Rush, an adrenaline-fueled racing game created with React Three Fiber. Showcase your expert racing skills as you navigate each curve with finesse in this captivating minimalist world, setting new records with every race.
![](https://github.com/iaruso/circuit-rush/blob/legacy/public/static/github-cover.jpg)

### Features
- Game controls animation (also interactive)
- Minimalist race track with checkpoints and waypoints
- Vehicle with realistic physics
- Tons of cubes to hit that react with collision
- Three laps in time trial ~~to compete worldwide against other players~~
- Adjustable settings based on your preference

> __Note__  
> Mobile devices are currently not supported.  
> Firefox excels in performance, while Chromium-based browsers perform decently with GPU acceleration enabled.

> __Warning__  
> Hardware acceleration must be enabled in your browser.

### [Play it now!](https://circuit-rush.vercel.app/)
